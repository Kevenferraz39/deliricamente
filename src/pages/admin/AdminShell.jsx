...
function AdminPosts({ posts, onEdit, onNew, onDelete, onToggleStatus, onUpdateCover }) {
  ...
  const handleThumbClick = (postId) => { pendingId.current = postId; fileRef.current?.click(); };
  ...
  const handleFileChange = async (e) => {
    ...
    try { const url = await uploadImg(file); onUpdateCover(postId, url); }
  ...
  return (
    <div>
      ...
      <div className="admin-head">
        <div><div className="kicker">// CONTEUDO</div><h1>Posts</h1></div>
        <Btn variant="red" arrow onClick={onNew}><Icon.Plus /> Novo post</Btn>
      </div>
      <div className="admin-table">
        <div className="admin-table-head">
          <span>TITULO</span><span>AUTOR</span><span>STATUS</span><span>CURTIDAS</span><span>COMENT.</span><span>ACOES</span>
        </div>
        {posts.map(p => (
          <div className="admin-table-row" key={p.id}>
            <div className="title-cell">
              <div className="title-thumb" style={{overflow:'hidden',cursor:'pointer',position:'relative',flexShrink:0}}
                onClick={()=>handleThumbClick(p.id)} onMouseEnter={()=>setHoverThumb(p.id)} onMouseLeave={()=>setHoverThumb(null)} title="Clique para trocar a imagem de capa">
                {uploadingId === p.id
                  ...
                  : p.cover?.url ? <img src={p.cover.url} alt="" style={{width:'100%',height:'100%',objectFit:'cover',display:'block'}} /> : <Placeholder label="" variant={p.cover?.variant} />
                }
                {hoverThumb === p.id && uploadingId !== p.id && (
                  <div style={{position:'absolute',inset:0,background:'rgba(0,0,0,0.6)',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'var(--font-mono)',fontSize:'0.55rem',color:'white'}}>TROCAR</div>
                )}
              </div>
              <div>
                <div>{p.title}</div>
...
function AdminEditor({ post, onSave, onCancel }) {
  ...
  const [draft, setDraft] = React.useState(post || {
    ...
    type: "EVENTO", title: "", excerpt: "",
    cover: { label: "NOVA CAPA", variant: "red" },
    date: new Date().toISOString().slice(0,10),
    author: "MC Roma", likes: 0, fires: 0, comments: 0, views: 0,
    tags: [], status: "draft", body: [{ kind: "p", text: "" }],
  });
  ...
  const handleCoverUpload = async (e) => {
    ...
    try { const url = await uploadImg(file); setDraft(d => ({ ...d, cover: { ...d.cover, url } })); }
  ...
  const parseBody = (txt) => {
    ...
    txt.split(/\n{2,}/).forEach(chunk => {
      ...
      else if (t.match(/^\[VIDEO/i)) { const url = t.replace(/^\[VIDEO:\s*/i,'').replace(/\]$/,'').trim(); blocks.push({ kind: "embed", url, label: t.replace(/\[|\]/g,"") }); }
      else if (t.split("\n").every(l => l.startsWith("- "))) blocks.push({ kind: "ul", items: t.split("\n").map(l => l.slice(2)) });
      else blocks.push({ kind: "p", text: t });
    });
    return blocks;
  };

  const save = (status) => { onSave({ ...draft, body: parseBody(bodyText), status }); };
    ...
    <div>
      ...
      <div className="editor-shell">
        <div className="editor-main">
          <input className="editor-title" placeholder="TÍTULO DO POST..." value={draft.title} onChange={e=>setDraft({...draft,title:e.target.value})} />
          <input className="input" style={{width:"100%",marginBottom:16}} placeholder="RESUMO (APARECE NA LISTA DE POSTS)" value={draft.excerpt} onChange={e=>setDraft({...draft,excerpt:e.target.value})} />
          ...
          <div className="editor-toolbar">
            ...
            <span style={{flex:1}} />
            <span className="mono" style={{color:"var(--muted)",alignSelf:"center",fontSize:'0.72rem'}}>**negrito** · *italico* · ## H2 · &gt; quote · - item</span>
          </div>

          <textarea ref={bodyRef} className="editor-body" value={bodyText} onChange={e=>setBodyText(e.target.value)} placeholder="Escreve o conteudo aqui..." />
          ...
          <div style={{marginTop:24,padding:20,background:"var(--black)",border:"1px solid var(--line)"}}>
            ...
            <h2 style={{fontFamily:"var(--display)",fontSize:36,lineHeight:0.95,textTransform:"uppercase",margin:"0 0 16px"}}>{draft.title || "TÍTULO DO POST..."}</h2>
            ...
            <div style={{borderTop:"1px solid var(--line)",marginTop:16,paddingTop:16}}>
              {parseBody(bodyText).slice(0,3).map((b,i)=>{
                ...
                if (b.kind === "embed") {
                  ...
                  return ytId
                    ? <div key={i} style={{position:'relative',paddingBottom:'56.25%',height:0,overflow:'hidden',margin:'8px 0'}}><iframe src={'https://www.youtube.com/embed/'+ytId} style={{position:'absolute',top:0,left:0,width:'100%',height:'100%',border:'none'}} allowFullScreen /></div>
        ...
        <div className="editor-side">
          <div className="box">
            ...
            <select className="input" style={{width:"100%"}} value={draft.type} onChange={e=>setDraft({...draft,type:e.target.value})}>
              <option>EVENTO</option><option>SHOW</option><option>NOTÍCIA</option><option>CULTURA</option><option>ANÚNCIO</option>
            </select>
          </div>
          <div className="box">
            <h4>// PUBLICAÇÃO</h4>
          ...
          <div className="box">
            ...
            <div className="upload-zone" style={{marginBottom:10,cursor:'pointer',padding:0,overflow:'hidden',minHeight:80}} onClick={()=>coverRef.current?.click()}>
              {coverUploading ? <div style={{padding:24,fontFamily:'var(--font-mono)',fontSize:'0.8rem'}}>Enviando...</div>
                : draft.cover?.url ? <img src={draft.cover.url} alt="capa" style={{width:'100%',height:120,objectFit:'cover',display:'block'}} />
          ...
          <div className="box">
            ...
            <div className="tag-input">
              {draft.tags.map(t => (
                <span className="t" key={t} onClick={() => setDraft({...draft, tags: draft.tags.filter(x => x !== t)})}>{t} <Icon.Close /></span>
              ))}
              <input placeholder="+ TAG" value={tagInput} onChange={e=>setTagInput(e.target.value)} onKeyDown={e=>{ if (e.key === "Enter" || e.key === ",") { e.preventDefault(); addTag(); } }} />
            </div>
          </div>
        </div>
      </div>
...
function AdminComments({ comments, onModerate }) {
  const all = [];
  Object.entries(comments).forEach(([postId, list]) => { list.forEach((c, idx) => all.push({ ...c, postId, idx })); });
  ...
  const CommentRow = ({ c, showFlagInfo = false }) => (
    <div style={{padding:"20px 24px",borderBottom:"1px solid var(--line)",display:"grid",gridTemplateColumns:"1fr auto",gap:24,alignItems:"start",background: showFlagInfo ? 'rgba(225,6,0,0.04)' : 'transparent'}}>
      <div>
        ...
        {showFlagInfo && c.flaggedTerms && (
          <div style={{marginTop:8,padding:'6px 10px',background:'rgba(225,6,0,0.12)',border:'1px solid rgba(225,6,0,0.3)',borderRadius:4}}>
            <span className="mono" style={{fontSize:'0.72rem',color:'var(--red)'}}>TERMOS DETECTADOS: {c.flaggedTerms.join(', ')}</span>
            {c.flaggedAt && <span className="mono" style={{fontSize:'0.68rem',color:'var(--muted)',marginLeft:12}}>bloqueado em {new Date(c.flaggedAt).toLocaleString('pt-BR')}</span>}
          </div>
        )}
      </div>
      <div className="row-actions" style={{flexDirection:'column',gap:4}}>
        {c.status === "pending" && <button className="icon-btn" title="Aprovar" onClick={()=>onModerate(c.postId,c.idx,"approved")} style={{borderColor:"#22c55e",color:"#22c55e"}}>&#10003;</button>}
  ...
  return (
    <div>
      <div className="admin-head">
        <div><div className="kicker">// MODERAÇÃO</div><h1>Comentários</h1></div>
        <div className="tag-chips">
          ...
          <button className={"chip " + (tab === "flagged"  ? "active" : "")} onClick={()=>setTab("flagged")} style={{borderColor: flagged.length ? 'var(--red)' : '', color: flagged.length ? 'var(--red)' : ''}}>Bloqueados · {flagged.length}</button>
        </div>
      </div>
      ...
      <div className="admin-table">
        {list.length === 0 && <div style={{padding:40,textAlign:"center",color:"var(--muted)"}} className="mono">// NENHUM COMENTÁRIO {tab === "pending" ? "PENDENTE" : tab === "approved" ? "APROVADO" : "BLOQUEADO"}</div>}
        {list.map((c) => <CommentRow key={c.postId+"-"+c.idx} c={c} showFlagInfo={tab === "flagged"} />)}
      </div>
    </div>
  );
}
...
function AdminGaleria() {
  ...
  return (
    <div>
      <div className="admin-head"><div><div className="kicker">// FOTOS E VIDEOS</div><h1>Galeria</h1></div></div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 24 }}>
        <div>
          {items.length === 0 && <div style={{ border: '1px dashed var(--gray)', padding: '3rem', textAlign: 'center', fontFamily: 'var(--font-mono)', color: 'var(--muted)' }}>Nenhuma foto ainda.</div>}
        ...
        <div className="editor-side">
          <div className="box">
            ...
            <input className="input" placeholder="Legenda" value={form.label} onChange={e => setF('label', e.target.value)} style={{ padding: '0.4rem', fontFamily: 'var(--font-mono)', fontSize: '0.8rem', marginBottom: 8, width: '100%' }} />
            <select className="input" value={form.size} onChange={e => setF('size', e.target.value)} style={{ padding: '0.4rem', fontFamily: 'var(--font-mono)', fontSize: '0.8rem', marginBottom: 4, width: '100%' }}>
              <option value="">Normal</option><option value="wide">Largo (2 colunas)</option><option value="tall">Alto (2 linhas)</option><option value="wide tall">Destaque</option>
            </select>
            <Btn variant="red" arrow onClick={addItem} disabled={saving || !form.url} style={{ width: '100%', justifyContent: 'center' }}>{saving ? 'SALVANDO...' : 'ADICIONAR'}</Btn>
          </div>
        </div>
      </div>
    </div>
...
function AdminTimeline() {
  ...
  React.useEffect(() => {
    ...
    return onSnapshot(q, snap => {
      ...
      if (docs.length > 0) { setItems(docs); }
        ...
        const seed = TIMELINE.map((t, i) => ({ ...t, id: 'seed-' + i, _isSeed: true }));
  ...
  const startEdit = (item) => { setEditId(item.id); setForm({ year: item.year || '', title: item.title || '', body: item.body || '', tags: (item.tags || []).join(', '), imageUrl: item.imageUrl || '' }); };
  const deleteItem = async (id) => { if (window.confirm('Remover?')) await deleteDoc(doc(db, 'timeline', id)); };

  return (
    <div>
      <div className="admin-head"><div><div className="kicker">// HISTORIA DO COLETIVO</div><h1>Timeline</h1></div></div>
        ...
        <div className="editor-side">
          <div className="box">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <textarea className="input textarea" placeholder="Descricao..." value={form.body} onChange={e => setF('body', e.target.value)} style={{ padding: '0.5rem', minHeight: 90, fontFamily: 'var(--font-body)', fontSize: '0.85rem' }} />
              ...
              <div>
                ...
                <div onClick={() => imgRef.current?.click()} style={{ border: '1px dashed var(--gray)', cursor: 'pointer', overflow: 'hidden', minHeight: 60, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 6 }}>
                  {uploading ? <span className="mono" style={{ fontSize: '0.8rem' }}>Enviando...</span> : form.imageUrl ? <img src={form.imageUrl} alt="" style={{ width: '100%', maxHeight: 100, objectFit: 'cover' }} /> : <span className="mono" style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Upload imagem</span>}
                </div>
...
function AdminLoja() {
  ...
  const deleteItem = async (id) => { if (window.confirm('Remover produto?')) await deleteDoc(doc(db, 'products', id)); };
  const toggleActive = async (item) => { await updateDoc(doc(db, 'products', item.id), { active: !item.active }); };

  return (
    <div>
      <div className="admin-head"><div><div className="kicker">// PRODUTOS DO COLETIVO</div><h1>Loja</h1></div></div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 24 }}>
        <div>
          {items.map(item => (
            <div key={item.id} style={{ display: 'flex', gap: 14, padding: '1rem', background: 'var(--panel)', border: `1px solid ${item.active !== false ? 'var(--line)' : 'rgba(225,6,0,0.3)'}`, marginBottom: 8, alignItems: 'center', opacity: item.active === false ? 0.6 : 1 }}>
              ...
              <div style={{ display: 'flex', gap: 4, flexShrink: 0, flexDirection: 'column' }}>
                <button className="icon-btn" onClick={() => startEdit(item)}>&#9998;</button>
                <button className="icon-btn" onClick={() => toggleActive(item)} style={{ fontSize: 10, fontFamily: 'var(--font-mono)' }}>{item.active !== false ? 'ON' : 'OFF'}</button>
                <button className="icon-btn danger" onClick={() => deleteItem(item.id)}>&#10005;</button>
              </div>
            </div>
          ))}
          ...
          <div className="box">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div onClick={() => imgRef.current?.click()} style={{ border: '1px dashed var(--gray)', cursor: 'pointer', overflow: 'hidden', height: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {uploading ? <span className="mono" style={{ fontSize: '0.8rem' }}>Enviando...</span> : form.imageUrl ? <img src={form.imageUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span className="mono" style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Foto do produto</span>}
              </div>
              ...
              <input className="input" placeholder="Link externo" value={form.link} onChange={e => setF('link', e.target.value)} style={{ padding: '0.5rem', fontFamily: 'var(--font-mono)', fontSize: '0.82rem' }} />
              <div style={{ display: 'flex', gap: 8 }}>
                <Btn variant="red" arrow onClick={saveItem} disabled={saving} style={{ flex: 1, justifyContent: 'center' }}>{saving ? 'SALVANDO...' : editId ? 'ATUALIZAR' : 'ADICIONAR'}</Btn>
                {editId && <button className="logout" onClick={() => { setEditId(null); setForm(EMPTY); }}>CANCELAR</button>}
              </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 64, height: 64, borderRadius: '50%', overflow: 'hidden', border: '2px solid var(--red)', flexShrink: 0, cursor: 'pointer' }} onClick={() => imgRef.current?.click()}>
              {form.photoUrl ? <img src={form.photoUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--gray)', fontFamily: 'var(--font-display)', fontSize: 22 }}>{(form.displayName || '?')[0]}</div>}
            </div>
            <div className="mono" style={{ fontSize: '0.7rem', color: 'var(--muted)', lineHeight: 1.5 }}>{uploading ? 'Enviando...' : 'Clique na foto para trocar'}</div>
          </div>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14 }}>
            <div style={{ width: 56, height: 56, borderRadius: '50%', overflow: 'hidden', border: '2px solid var(--gray)', flexShrink: 0 }}>
              {(myDoc.photoUrl || form.photoUrl) ? <img src={myDoc.photoUrl || form.photoUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--panel)', fontFamily: 'var(--font-display)', fontSize: 22, color: 'var(--red)' }}>{(user?.name || '?')[0].toUpperCase()}</div>}
            </div>
            <div>
...
function AdminSettings({ user }) {
  ...
  const saveUser = async (uid) => { await updateDoc(doc(db, 'users', uid), editForm); setEditingUid(null); };
  const toggleActive = async (u) => { if (!isMaster || u.uid === MASTER_ADMIN_UID) return; await updateDoc(doc(db, 'users', u.uid), { active: !u.active }); };

  return (
    <div>
      <div className="admin-head">
        <div><div className="kicker">// EQUIPE</div><h1>Usuarios & Ajustes</h1></div>
        {isMaster && <div className="mono" style={{ color: 'var(--muted)', fontSize: '0.8rem' }}>Voce e o Admin Master — pode definir funcoes e status de todos os membros.</div>}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 24 }}>
        <div className="admin-table">
              ...
              {editingUid === u.uid ? (
                <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--line)', background: 'rgba(225,6,0,0.04)' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
                    ...
                    <input className="input" placeholder="URL da foto" value={editForm.photoUrl || ''} onChange={e => setEditForm(f => ({ ...f, photoUrl: e.target.value }))} style={{ padding: '0.4rem', fontFamily: 'var(--font-mono)', fontSize: '0.8rem' }} />
                  </div>
                ...
                <div className="admin-table-row" style={{ gridTemplateColumns: '1.5fr 1.2fr 1fr 90px 80px' }}>
                  <div className="title-cell">
                    <div className="comment-avatar" style={{ width: 36, height: 36, fontSize: 14, overflow: 'hidden', padding: 0 }}>
                      {u.photoUrl ? <img src={u.photoUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : (u.displayName || u.email || '?')[0].toUpperCase()}
                    </div>
                    <div>{u.displayName || u.email?.split('@')[0]}{u.uid === MASTER_ADMIN_UID && <span className="mono" style={{ fontSize: '0.6rem', color: 'var(--red)', marginLeft: 6 }}>MASTER</span>}</div>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 4 }}>
                    {isMaster && u.uid !== MASTER_ADMIN_UID && (
                      <>
                        ...
                        <button className="icon-btn" onClick={() => toggleActive(u)} style={{ fontSize: '0.65rem', fontFamily: 'var(--font-mono)', color: u.active !== false ? '#22c55e' : 'var(--muted)' }}>{u.active !== false ? 'ON' : 'OFF'}</button>
                      </>
                    )}
                  </div>
                </div>
        ...
        <div className="editor-side">
          ...
          <div className="box">
            <h4>// SITE</h4>
            <div className="mono" style={{ lineHeight: 1.7, color: 'var(--paper)' }}>VERSAO · v1.0<br/>MEMBROS · {users.length}</div>
          </div>
        </div>
      </div>
    </div>
...
// ---------- TEMA ----------
const GOOGLE_FONTS_LIST = ['Anton','Bebas Neue','Oswald','Barlow Condensed','Righteous','Russo One','Teko','Syncopate','Syne','Bungee','Orbitron','Inter','Roboto','Open Sans','Poppins','Montserrat','DM Sans','Space Grotesk','Outfit','Figtree','Merriweather','Playfair Display','Lora','JetBrains Mono','Fira Code','Source Code Pro','Space Mono','Dancing Script','Caveat','Permanent Marker'].sort();
...
function FontPicker({ value, onChange, label, allFonts }) {
  ...
  const filtered = React.useMemo(() => allFonts.filter(f => f.toLowerCase().includes(search.toLowerCase())).slice(0, 40), [allFonts, search]);
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      <input type="text" className="input" value={search} placeholder="Buscar fonte..."
      ...
      {open && filtered.length > 0 && (
        <div style={{ position: 'absolute', zIndex: 200, width: '100%', top: '100%', background: 'var(--panel)', border: '1px solid var(--gray)', maxHeight: 220, overflowY: 'auto', boxShadow: '0 8px 24px rgba(0,0,0,.6)' }}>
          {filtered.map(f => (
            <div key={f} onClick={() => select(f)} style={{ padding: '7px 12px', cursor: 'pointer', fontSize: 14, color: f === value ? 'var(--red)' : 'var(--off-white)', background: f === value ? 'rgba(225,6,0,0.08)' : 'transparent', borderBottom: '1px solid var(--line)', fontFamily: 'var(--font-mono)' }}
              onMouseEnter={e => e.currentTarget.style.background = 'var(--gray)'}
              onMouseLeave={e => e.currentTarget.style.background = f === value ? 'rgba(225,6,0,0.08)' : 'transparent'}>{f}</div>
          ))}
        </div>
      )}
    </div>
...
function AdminThemeEditor({ user }) {
  const DEFAULTS = { accent: '#E10600', background: '#0A0A0A', offWhite: '#F4F8EB', panel: '#1A1A1A', gray: '#2A2A2A', muted: '#888888', paper: '#E5E0D8', textBody: '#bbbbbb', heroLogoUrl: '', navLogoUrl: 'https://i.ibb.co/nM8qGYxn/images-removebg-preview.png', fontDisplay: 'Anton', fontBody: 'Space Grotesk', fontMono: 'JetBrains Mono' };
  ...
  React.useEffect(() => {
    const unsub = onSnapshot(doc(db, 'config', 'theme'), (snap) => { if (snap.exists()) setTheme(t => ({ ...DEFAULTS, ...snap.data() })); });
    return () => unsub();
  }, []);
  ...
  React.useEffect(() => {
    ...
    if (!key) return;
    fetch(`https://www.googleapis.com/webfonts/v1/webfonts?key=${key}&sort=popularity`).then(r => r.json()).then(data => { if (data.items) setAllFonts(data.items.map(f => f.family)); }).catch(() => {});
  }, []);
  ...
  const notify = (text, ok = true) => { setMsg({ text, ok }); setTimeout(() => setMsg({ text: '', ok: true }), 5000); };

  const handleSave = async () => {
    setSaving(true);
    try { await setDoc(doc(db, 'config', 'theme'), { ...theme, updatedAt: serverTimestamp(), updatedBy: user?.email || 'admin' }); notify('Tema salvo! Todos os usuários já estão vendo as mudanças.'); }
    catch (e) { notify('Erro ao salvar: ' + e.message, false); }
    setSaving(false);
  };

  const handleReset = async () => {
    if (!window.confirm('Restaurar o tema padrão?')) return;
    setTheme(DEFAULTS);
    await setDoc(doc(db, 'config', 'theme'), { ...DEFAULTS, updatedAt: serverTimestamp(), updatedBy: user?.email || 'admin' });
  ...
  const colorFields = [
    ...
    { key: 'muted', label: 'Texto Secundário', desc: '--muted' },
  ...
  return (
    <div>
      <div className="admin-head">
        <div><div className="kicker">// APARÊNCIA DO SISTEMA</div><h1>Tema</h1></div>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          <button className="logout" onClick={handleReset}>RESTAURAR PADRÃO</button>
          <Btn variant="red" arrow onClick={handleSave} disabled={saving}>{saving ? 'SALVANDO...' : 'SALVAR E APLICAR'}</Btn>
        </div>
      </div>
      {msg.text && <div style={{ background: msg.ok ? 'rgba(34,197,94,0.08)' : 'rgba(225,6,0,0.08)', border: `1px solid ${msg.ok ? 'rgba(34,197,94,0.3)' : 'rgba(225,6,0,0.3)'}`, color: msg.ok ? '#22c55e' : 'var(--red)', padding: '1rem 1.5rem', fontFamily: 'var(--font-mono)', fontSize: '0.85rem', marginBottom: 24 }}>{msg.ok ? '✓' : '✕'} {msg.text}</div>}
              ...
              {colorFields.map(({ key, label, desc }) => (
                <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  <div style={{ position: 'relative', width: 44, height: 44, flexShrink: 0 }}>
                    ...
                    <input type="color" value={theme[key]} onChange={e => set(key, e.target.value)} style={{ position: 'absolute', inset: 0, opacity: 0, cursor: 'pointer', width: '100%', height: '100%' }} />
                  </div>
                  <div style={{ flex: 1 }}>
                    ...
                    <input type="text" className="input" value={theme[key]} onChange={e => set(key, e.target.value)} style={{ padding: '0.4rem 0.6rem', fontFamily: 'var(--font-mono)', fontSize: '0.8rem' }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="editor-side">
            <div className="box">
              ...
              <div style={{ marginBottom: 16 }}>
                <div className="mono" style={{ fontSize: '0.72rem', color: 'var(--muted)', marginBottom: 6 }}>IMAGEM DO HERO</div>
                <input type="text" className="input" placeholder="URL da imagem" value={theme.heroLogoUrl || ''} onChange={e => set('heroLogoUrl', e.target.value)} style={{ padding: '0.5rem', fontFamily: 'var(--font-mono)', fontSize: '0.8rem', marginBottom: 6 }} />
                {theme.heroLogoUrl && <><img src={theme.heroLogoUrl} alt="hero preview" style={{ maxHeight: 80, maxWidth: '100%', objectFit: 'contain', display: 'block', border: '1px solid var(--line)' }} /><button className="logout" style={{ marginTop: 4, width: '100%' }} onClick={() => set('heroLogoUrl', '')}>REMOVER</button></>}
              </div>
              <div style={{ marginBottom: 16 }}>
                <div className="mono" style={{ fontSize: '0.72rem', color: 'var(--muted)', marginBottom: 6 }}>LOGO DA NAVBAR</div>
                <input type="text" className="input" placeholder="URL do logo" value={theme.navLogoUrl || ''} onChange={e => set('navLogoUrl', e.target.value)} style={{ padding: '0.5rem', fontFamily: 'var(--font-mono)', fontSize: '0.8rem', marginBottom: 6 }} />
                {theme.navLogoUrl && <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}><img src={theme.navLogoUrl} alt="" style={{ height: 40, objectFit: 'contain' }} /><button className="logout" style={{ flex: 1 }} onClick={() => set('navLogoUrl', '')}>REMOVER</button></div>}
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                {[{ key: 'fontDisplay', label: 'FONTE DE TÍTULOS' }, { key: 'fontBody', label: 'FONTE DE TEXTOS' }, { key: 'fontMono', label: 'FONTE MONO/TAGS' }].map(({ key, label }) => (
                  <div key={key}>
                    <FontPicker value={theme[key]} onChange={v => set(key, v)} label={label} allFonts={allFonts} />
                    <div style={{ marginTop: 6, padding: '8px 12px', background: 'var(--black)', border: '1px solid var(--line)', fontFamily: `'${theme[key]}', sans-serif`, fontSize: 18, color: 'var(--off-white)' }}>Deliricamente</div>
                  </div>
                ))}
              </div>
            </div>
...
function AdminBackground({ user }) {
  ...
  React.useEffect(() => {
    const unsub = onSnapshot(doc(db, 'config', 'background'), snap => { if (snap.exists()) setCfg(c => ({ ...DEFAULTS, ...snap.data() })); });
    return () => unsub();
  }, []);

  const save = async (updated) => {
    setSaving(true);
    try { await setDoc(doc(db, 'config', 'background'), { ...updated, updatedAt: serverTimestamp(), updatedBy: user?.email }); setMsg('Salvo!'); }
  ...
  const styles = [
    { id: 'blobs', label: 'Manchas', icon: 'O', desc: 'Circulos flutuantes' },
    { id: 'rede', label: 'Rede', icon: '+', desc: 'Particulas conectadas' },
    { id: 'geo', label: 'Geometrico', icon: '^', desc: 'Formas wireframe' },
    { id: 'glitch', label: 'Glitch', icon: '#', desc: 'Corrupcao digital' },
    { id: 'chuva', label: 'Chuva', icon: '|', desc: 'Chuva de caracteres' },
    { id: 'spray', label: 'Spray', icon: '.', desc: 'Tinta em spray' },
    { id: 'pichacao', label: 'Pichacao', icon: '/', desc: 'Tags em movimento' },
    { id: 'off', label: 'Desligado', icon: 'X', desc: 'Fundo solido' },
  ];

  return (
    <div>
      <div className="admin-head">
        <div><div className="kicker">// APARENCIA</div><h1>Fundo Animado</h1></div>
        <Btn variant="red" arrow onClick={() => save(cfg)} disabled={saving}>{saving ? 'SALVANDO...' : 'SALVAR E APLICAR'}</Btn>
      </div>
      {msg && <div style={{ background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.3)', color: '#22c55e', padding: '1rem 1.5rem', fontFamily: 'var(--font-mono)', fontSize: '0.85rem', marginBottom: 24 }}>{msg}</div>}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        <div className="editor-side">
          <div className="box">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginTop: 8 }}>
              {styles.map(s => (
                <div key={s.id} onClick={() => set('style', s.id)} style={{ padding: '1rem', border: `1px solid ${cfg.style === s.id ? 'var(--red)' : 'var(--line)'}`, background: cfg.style === s.id ? 'rgba(225,6,0,0.08)' : 'var(--black)', cursor: 'pointer', textAlign: 'center' }}>
        ...
        <div className="editor-side">
          <div className="box">
            <h4>PARAMETROS</h4>
            {[{ key: 'speed', label: 'VELOCIDADE', min: 0.1, max: 3, step: 0.1 }, { key: 'density', label: 'DENSIDADE', min: 3, max: 40, step: 1 }, { key: 'opacity', label: 'OPACIDADE', min: 0.1, max: 1, step: 0.05 }].map(({ key, label, min, max, step }) => (
              <div key={key} style={{ marginBottom: 16 }}>
                ...
                <input type="range" min={min} max={max} step={step} value={cfg[key]} onChange={e => set(key, parseFloat(e.target.value))} style={{ width: '100%', accentColor: 'var(--red)' }} />
              </div>
            ))}
          </div>
        </div>
...
function AdminCarousel({ user }) {
  ...
  const handleUpload = async (e) => {
    const file = e.target.files[0]; if (!file) return;
    const key = import.meta.env.VITE_IMGBB_KEY;
    if (!key || key === 'SUA_CHAVE_IMGBB_AQUI') { setUploadErr('Adicione VITE_IMGBB_KEY no .env'); e.target.value = ''; return; }
    ...
    } catch (err) { setUploadErr('Erro: ' + err.message); }
    setUploading(false); e.target.value = '';
  };

  React.useEffect(() => {
    const unsub = onSnapshot(doc(db, 'config', 'carousel'), snap => { if (snap.exists()) setCfg(c => ({ ...c, ...snap.data() })); });
    return () => unsub();
  }, []);

  const persist = async (updated) => {
    setSaving(true);
    try { await setDoc(doc(db, 'config', 'carousel'), { ...updated, updatedAt: serverTimestamp(), updatedBy: user?.email }); setMsg('Salvo!'); }
    catch (e) { setMsg('Erro: ' + e.message); }
  ...
  const saveSlide = () => {
    ...
    const slides = editIdx !== null ? cfg.slides.map((s, i) => i === editIdx ? { ...form, id: s.id } : s) : [...cfg.slides, { ...form, id: Date.now().toString() }];
    const next = { ...cfg, slides }; setCfg(next); persist(next);
    setForm(SLIDE_EMPTY); setEditIdx(null);
  };
  const deleteSlide = (idx) => { if (!window.confirm('Remover slide?')) return; const slides = cfg.slides.filter((_, i) => i !== idx); const next = { ...cfg, slides }; setCfg(next); persist(next); };
  const moveSlide = (idx, dir) => { const slides = [...cfg.slides]; const target = idx + dir; if (target < 0 || target >= slides.length) return; [slides[idx], slides[target]] = [slides[target], slides[idx]]; const next = { ...cfg, slides }; setCfg(next); persist(next); };
  ...
  return (
    <div>
      <div className="admin-head">
        <div><div className="kicker">// NOVIDADES EM DESTAQUE</div><h1>Carrossel</h1></div>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          <span className="mono" style={{ fontSize: '0.8rem', color: cfg.enabled ? '#22c55e' : 'var(--muted)' }}>{cfg.enabled ? 'VISIVEL' : 'OCULTO'}</span>
          <button onClick={toggleEnabled} style={{ padding: '0.6rem 1.4rem', fontFamily: 'var(--font-display)', fontSize: '1rem', textTransform: 'uppercase', cursor: 'pointer', background: cfg.enabled ? 'rgba(225,6,0,0.1)' : 'var(--red)', color: cfg.enabled ? 'var(--red)' : 'var(--black)', border: `1px solid ${cfg.enabled ? 'var(--red)' : 'transparent'}` }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <label style={{ fontFamily: 'var(--font-mono)', fontSize: '0.8rem', color: 'var(--muted)', display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
                <input type="checkbox" checked={cfg.autoPlay !== false} onChange={e => { const next = { ...cfg, autoPlay: e.target.checked }; setCfg(next); persist(next); }} style={{ accentColor: 'var(--red)' }} />AUTO-PLAY
              </label>
            </div>
          </div>
          {cfg.slides.length === 0 && <div style={{ border: '1px dashed var(--gray)', padding: '3rem', textAlign: 'center', fontFamily: 'var(--font-mono)', color: 'var(--muted)', fontSize: '0.85rem' }}>Nenhum slide ainda.</div>}
          {cfg.slides.map((sl, i) => (
            <div key={sl.id || i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', padding: '1rem', background: 'var(--panel)', border: '1px solid var(--line)', marginBottom: 8 }}>
              {sl.imageUrl ? <div style={{ width: 80, height: 55, flexShrink: 0, backgroundImage: `url(${sl.imageUrl})`, backgroundSize: 'cover', backgroundPosition: 'center', border: '1px solid var(--gray)' }} /> : <div style={{ width: 80, height: 55, flexShrink: 0, background: 'var(--gray)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-mono)', fontSize: '0.65rem', color: 'var(--muted)' }}>SEM IMG</div>}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flexShrink: 0 }}>
                <div style={{ display: 'flex', gap: 4 }}>
                  <button className="icon-btn" onClick={() => moveSlide(i, -1)} disabled={i === 0}>&#8679;</button>
                  <button className="icon-btn" onClick={() => moveSlide(i, 1)} disabled={i === cfg.slides.length - 1}>&#8681;</button>
                  <button className="icon-btn" onClick={() => startEdit(i)}>&#9998;</button>
                  <button className="icon-btn danger" onClick={() => deleteSlide(i)}>&#10005;</button>
                </div>
              </div>
            </div>
          ))}
          ...
          <div className="box">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div>
                ...
                <input ref={fileRef} type="file" accept="image/*" style={{ display: 'none' }} onChange={handleUpload} />
                <div onClick={() => fileRef.current?.click()} style={{ border: '1px dashed var(--gray)', padding: '0.75rem 1rem', cursor: 'pointer', textAlign: 'center', fontFamily: 'var(--font-mono)', fontSize: '0.78rem', color: uploading ? 'var(--red)' : 'var(--muted)', marginBottom: 6 }}>
                  {uploading ? 'Enviando...' : 'Clique para fazer upload'}
                </div>
                {uploadErr && <div className="mono" style={{ color: 'var(--red)', fontSize: '0.72rem', marginBottom: 6 }}>{uploadErr}</div>}
                {form.imageUrl && <div style={{ width: '100%', height: 80, backgroundImage: 'url(' + form.imageUrl + ')', backgroundSize: 'cover', backgroundPosition: 'center', border: '1px solid var(--gray)', marginBottom: 6 }} />}
              ...
              <input className="input" placeholder="// KICKER (opcional)" value={form.kicker} onChange={e => setF('kicker', e.target.value)} style={{ padding: '0.5rem', fontFamily: 'var(--font-mono)', fontSize: '0.82rem' }} />
              ...
              <textarea className="input textarea" placeholder="Subtitulo (opcional)" value={form.subtitle} onChange={e => setF('subtitle', e.target.value)} style={{ padding: '0.5rem', fontFamily: 'var(--font-body)', fontSize: '0.85rem', minHeight: 70 }} />
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                <input className="input" placeholder="Texto do botao" value={form.ctaText} onChange={e => setF('ctaText', e.target.value)} style={{ padding: '0.5rem', fontFamily: 'var(--font-mono)', fontSize: '0.82rem' }} />
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                <Btn variant="red" arrow onClick={saveSlide} style={{ flex: 1, justifyContent: 'center' }}>{editIdx !== null ? 'ATUALIZAR' : 'ADICIONAR SLIDE'}</Btn>
                {editIdx !== null && <button className="logout" onClick={() => { setEditIdx(null); setForm(SLIDE_EMPTY); }}>CANCELAR</button>}
              </div>
            </div>
          </div>
        </div>
...
export default function AdminShell() {
  ...
  const { user, logout, posts, setPosts, comments, setComments } = useApp();
  ...
  const savePost = async (post) => {
    try { await setDoc(doc(db, 'posts', post.id), { ...post, updatedAt: serverTimestamp() }); }
    catch (e) {
      setPosts(prev => { const exists = prev.find(p => p.id === post.id); if (exists) return prev.map(p => p.id === post.id ? post : p); return [post, ...prev]; });
    }
  ...
  const updatePostCover = async (id, url) => {
    ...
    try { await updateDoc(doc(db, 'posts', id), { cover: newCover }); }
    catch (e) { setPosts(prev => prev.map(p => p.id === id ? { ...p, cover: newCover } : p)); }
  };

  const toggleStatus = async (id) => {
    const post = posts.find(p => p.id === id); if (!post) return;
    const newStatus = post.status === "published" ? "draft" : "published";
    try { await updateDoc(doc(db, 'posts', id), { status: newStatus }); }
    catch (e) { setPosts(prev => prev.map(p => p.id === id ? { ...p, status: newStatus } : p)); }
  };

  let content;
  if (section === "dashboard") content = <AdminDashboard posts={posts} comments={comments} user={user} />;
  else if (section === "posts") content = <AdminPosts posts={posts} onEdit={(id)=>{setEditingId(id); setSection("editor");}} onNew={()=>{setEditingId(null); setSection("editor");}} onDelete={deletePost} onToggleStatus={toggleStatus} onUpdateCover={updatePostCover} />;
  else if (section === "editor") content = <AdminEditor post={editingId ? posts.find(p => p.id === editingId) : null} onSave={savePost} onCancel={()=>{setEditingId(null); setSection("posts");}} />;
  ...
  const SidebarContent = ({ onNav }) => (
    <>
      ...
      <button className="logout" onClick={() => navigate('/')}>← VOLTAR AO SITE</button>
  ...
  return (
    <div className="admin-shell">
      ...
      <div className="admin-mobile-header">
        ...
        <button onClick={()=>setSidebarOpen(true)} style={{background:'transparent',border:'1px solid var(--line)',color:'var(--off-white)',padding:'6px 12px',cursor:'pointer',fontFamily:'var(--font-mono)',fontSize:'0.75rem'}}>MENU &#9776;</button>
      </div>
      {sidebarOpen && (
        <>
      ...
      <main className="admin-main page-enter">{content}</main>
    </div>
  );
}
